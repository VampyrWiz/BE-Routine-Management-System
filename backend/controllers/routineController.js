...
const { evaluateWorkload } = require('../utils/teacherWorkload');
...
exports.assignClass = async (req, res) => {
  ...
  try {
    ...
    // Weekly workload check: a teacher is allotted fifteen hours of teaching
    // per week by default. Pushing a teacher beyond that (or beyond their
    // individually configured limit) is permitted only for a user holding
    // the HoD role; a DHoD is declined and told to seek HoD approval.
    if (classType !== 'BREAK' && teacherIds && teacherIds.length > 0) {
      const additionalHours = (timeSlot.duration || 0) / 60;
      for (const singleTeacherId of teacherIds) {
        const workload = await evaluateWorkload({
          teacherId: singleTeacherId,
          academicYearId: academicCalendar._id,
          additionalHours,
          requestingUser: req.user,
          excludeSlotId: existingSlot ? existingSlot._id : null
        });

        if (!workload.allowed) {
          return res.status(403).json({
            success: false,
            message: `Assigning this class would raise ${workload.teacherName || 'the teacher'}'s weekly load to ${workload.projectedHours.toFixed(1)} hours, beyond the ${workload.weeklyLimit}-hour standard. Approval from the HoD is required for teaching loads beyond the weekly limit.`,
            requiresHodApproval: true,
            currentHours: workload.currentHours,
            projectedHours: workload.projectedHours,
            weeklyLimit: workload.weeklyLimit
          });
        }
      }
    }
...
exports.assignClassSpanned = async (req, res) => {
  ...
  try {
    ...
    // Weekly workload check: pushing a teacher beyond the fifteen-hour
    // standard load (or their configured limit) is permitted only for a
    // user holding the HoD role.
    {
      const spanTimeSlot = timeSlotMap.get(actualSlotIndexes[0]);
      const hoursPerPeriod = spanTimeSlot ? (spanTimeSlot.duration || 0) / 60 : 0;
      const additionalHours = hoursPerPeriod * actualSlotIndexes.length;
...
exports.exportAllTeachersSchedulesToPDF = async (req, res) => {
  try {
    ...
    const PDFRoutineService = require('../services/PDFRoutineService');
    const pdfService = new PDFRoutineService();
    const pdfBuffer = await pdfService.generateAllTeachersSchedulesPDF(semesterGroup);
