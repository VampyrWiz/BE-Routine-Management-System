...
// Middleware to check if authenticated user holds the HoD or DHoD role, but allows public access
exports.optionalAdmin = (req, res, next) => {
  ...
  // If user exists but holds neither the HoD nor the DHoD role, deny access
  if (req.user.role !== 'hod' && req.user.role !== 'dhod') {
    return res.status(403).json({
      success: false,
      message: 'HoD or DHoD access is required for this operation',
    });
  }
